        ...
        if (typeof window !== "undefined") {
            if (props && props.data) {
                ...
                const legs = props.data.map(el => {
                    return {
                        source: { lon: el.lngFrom, lat: el.latFrom },
                        destination: { lon: el.lngTo, lat: el.latTo },
                    };
                });
        ...
        if (journeys) {
            map.current.on("load", function() {
                journeys.forEach((journey,index) => {
                    ...
                    drawSourceAndDestination(journey, size, map.current, index);
                    drawLine(map.current, journey.source, journey.destination, index);
                });
            });
        }
    }, [journeys]);
...
const drawSourceAndDestination = (journeys, size, map, index) => {
    const pulsingDot = {
        width: size,
        height: size,
...
const drawLine = (map, startingPos, endingPos, index) => {
    ...
    map.addLayer({
        id: "line-animation"+String(index),
        type: "line",
        source: {
            type: "geojson",
            data: geojson,
    ...
    setInterval(() => {
        ...
        function animateLine() {
            ...
            if (animationCounter < lineCoordinates.length) {
                ...
                //update the map
                map.getSource("line-animation"+String(index)).setData(geojson);
